...
const getTickets = asyncHandler(async (req, res) => {
    try {
        ...
        const result = await db.executeQueryWithParams(query, params, "siri_db");
...
const previewTicketDocument = asyncHandler(async (req, res) => {
    ...
    const result = await db.executeQueryWithParams(
...
const updateTicketStatus = asyncHandler(async (req, res) => {
    try {
        ...
        `;
        const fetchResult = await db.executeQueryWithParams(
            fetchQuery,
            { ticket_id },
            "siri_db"
        );
        ...
        await db.executeQueryWithParams(updateQuery, updateParams, "siri_db");
...
const TicketStatusDD = asyncHandler(async (req, res) => {
    try {
        ...
        const result = await db.executeQueryWithParams(query, {}, "siri_db");
        const rows = result.rows || [];
...
const UpdateTicket = asyncHandler(async (req, res) => {
    try {
        ...
        if (req.files && req.files.length > 0) {
            ...
            const existingResult = await db.executeQueryWithParams(
                existingDocsQuery,
                { TICKET_ID },
                "siri_db"
            );
